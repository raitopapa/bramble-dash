...
## 難易度の全体的な引き下げ（第16弾）— こども向け
要望: 大ジャンプ（高い土管・広い穴）がシビアすぎる、敵を倒しやすく、全体的にやさしく。
- 地形:
  - `pipe()`/`warpPipe()` に **h=Math.min(h,2)**（土管は最大2マス。3〜4→2）。warp zone も同じhで整合。bonus出口土管もh2に統一。
  - 各builderの `pit` closure に **w=Math.min(w,2)**（陸/洞窟の穴は最大2マス。3〜5→2）。穴はタイルを埋める方向なので安全。
  - 空(3-x)の島間ギャップ(4)はそのまま＝物理で対応（走り幅~5.6タイルで余裕、歩きでも届く）。
- 物理(ふつう)を大幅にやさしく: jumpMul1.10 / gravityMul0.88 / fallMul0.92 / coyote×2 / buffer×2 / enemyMul0.80 / cutKeep0.62 / lives4 / timeMul0.8。
  やさしい(5さい)も更に緩和(jump1.16, grav0.82, coyote×3, enemy0.5, lives6)。
- 踏みつけ寛容化: `fromAbove = p.vy>0.2 && (p.y+p.h)-e.y < e.h*0.95`（弱い下降でも踏める＝敵を倒しやすい・上接触で被弾しにくい）。enemyMul低下でボス攻撃/突進も緩やか。
- テスト: 難易度テストのlives期待値を更新(age5=6)。全12面traverse＋ボーナス入退出グリーン。max pipe=2, 陸/洞窟 pit<=2 を確認。
